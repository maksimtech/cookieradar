"""
CookieRadar — local cache of EU law provisions.

Stored in ~/.cookieradar/law_cache.json (or $COOKIERADAR_HOME/law_cache.json):
...
from cookieradar.law_fetcher import Provision, text_sha256

Key = tuple[str, str]   # (celex, article)


def default_cache_path() -> Path:
    """Where the cache lives, from COOKIERADAR_HOME or the default under $HOME.
    ...
    """
    home = (os.environ.get("COOKIERADAR_HOME") or "").strip()
    if not home:
        return Path.home() / ".cookieradar" / "law_cache.json"
